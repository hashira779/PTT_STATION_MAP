...
fetch("https://raw.githubusercontent.com/hashira779/PTT_STATION_MAP/master/data/markers_admin.json")
...
function getIconUrl(status) {
  ...
  if (underConstruction) {
    ...
    console.log("Status: Brand Change");
    return "./pictures/brand_change.png"; // **Path to your brand change icon**
  } else {
...
function getStatusInfo(status) {
  ...
  if (lowerCaseStatus === "under construct") {
  ...
  } else if (lowerCaseStatus === "24h") {
    return {
      iconClass: "fa-gas-pump",
      badgeClass: "bg-success text-white",
      displayText: "Open 24h",
    };
  } else if (lowerCaseStatus === "brand change") {
    // --- New Condition Added ---
    console.log("Status: Brand Change");
    return {
    ...
    const openingHour = 5; // 5 AM
    const closingHour = 20; // 8 PM
    const closingMinutes = 30; // 8:30 PM
