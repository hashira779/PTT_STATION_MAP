...
fetch("https://raw.githubusercontent.com/hashira779/PTT_STATION_MAP/master/data/markers_admin_fleet.json")
...
function autoSelectFleetCard() {
  ...
  if (fleetCardIcon) {
    fleetCardIcon.classList.add('selected');
    console.log("Fleet card icon selected.");
    updateClearFilterButton(); // Update button visibility
    applyFilter(); // Apply the filter immediately
  } else {
    console.error("Fleet card icon not found in #service-icons.");
  }
}
